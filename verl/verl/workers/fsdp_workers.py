...
class ActorRolloutRefWorker(Worker, DistProfilerExtension): 
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="cyan", role="actor_compute_topk_ids")
    def compute_actor_topk_ids(self, data: DataProto):
        ...
        top_k = int(data.meta_info["top_k"])
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_actor_topk_ids", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="cyan", role="ref_compute_topk_ids")
    def compute_ref_topk_ids(self, data: DataProto):
        if self._is_lora:
            data.meta_info["is_lora"] = True
            output = self.compute_actor_topk_ids(data)
            return DataProto.from_dict(tensors={"ref_topk_ids": output.batch["actor_topk_ids"]})
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="cyan", role="base_compute_topk_ids")
    def compute_base_topk_ids(self, data: DataProto):
        if not self._has_base_model:
